<x-layout.layout>
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                <div class="flex flex-wrap gap-2 text-sm font-semibold" id="shiftButtons">
                    ...
                    <a id="{{ strtolower($shiftName) }}Link"
                        ...
                        href="{{ route('operation-officer.attendance-time-out-page', ['shift' => $shiftName, 'date' => $date]) }}">
                        {{ $shiftName }}
                    </a>
                    @endforeach
                </div>
                ...
                <table class="w-full text-xs font-[Roboto] min-w-[750px] text-left text-gray-600">
                    <thead class="bg-gray-100 text-gray-700 uppercase tracking-wider border-b border-gray-200">
                        <tr class="text-center">
                            ...
                            <th scope="col" class="px-5 py-3 font-bold text-center">Time-out</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="text-center">
                            ...
                            ->where('shift', $shift)
                            ->where('date', $date)
                            ->where('category', 'Time-out')
                            ->first();
                            @endphp

                                           <td class="py-2 px-3">
                                {{ $attendance 
        ? \Carbon\Carbon::parse($attendance->time_out)->timezone('Asia/Manila')->format('g:i A') 
        : 'No Record' 
    }}
                            </td>
                             <td class="py-2 px-3">
                                ...
                                'Time-out Success' => 'bg-green-600 text-white font-semibold rounded px-3 py-2',
                                'Missed Time-out' => 'bg-yellow-600 text-white font-semibold rounded px-3 py-2',
                            ...
                            <td class="py-2 px-3 flex flex-row justify-center item-center gap-2">
                                @if (!$attendance)
                                {{-- Time-out --}}
                                <form action="{{ route('operation-officer.attendance-time-out') }}" method="POST">
                                    @csrf
                                    <input type="hidden" name="user_id" value="{{ $user->id }}">
                                    <input type="hidden" name="shift" value="{{ $shift }}">
                                    <input type="hidden" name="date" value="{{ $date }}">
                                    <input type="hidden" name="time_out" value="{{ now('Asia/Manila')->format('H:i:s') }}">
                                    <input type="hidden" name="category" value="Time-out">
                                    <input type="hidden" name="status" value="Time-out Success">
                                    <button class="px-3 py-1 bg-orange-600 text-white rounded">Time-out</button>
                                </form>

                                {{-- Missed Time-out --}}
                                <form action="{{ route('operation-officer.attendance-missed-time-out') }}" method="POST">
                                    @csrf
                                    <input type="hidden" name="user_id" value="{{ $user->id }}">
                                    <input type="hidden" name="shift" value="{{ $shift }}">
                                    <input type="hidden" name="date" value="{{ $date }}">
                                    <input type="hidden" name="category" value="Time-out">
                                    <input type="hidden" name="status" value="Missed Time-out">
                                    <button class="px-3 py-1 bg-red-600 text-white rounded">Missed Time-out</button>
                                </form>
                                @else
    ...
    <script>
        ...
        dateInput.addEventListener('change', function() {
            ...
            window.location.href = `/operation-officer/attendance/Time-out/${currentShift}/${selectedDate}`;
        });
    </script>
