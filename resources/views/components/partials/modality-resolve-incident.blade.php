<x-layout.layout>
    ...
    <!-- Load Tailwind CSS (should ideally be in your layout file, but included here for standalone use) -->
    <script src="https://cdn.tailwindcss.com"></script>
    <!-- Use Inter font (should ideally be in your layout file) -->
    <style>
    ...
    <div class="max-w-5xl mx-auto bg-white p-6 md:p-10 rounded-xl shadow-2xl border border-gray-100">
        ...
        {{-- Laravel/Blade form setup. Action should be updated to a route --}}
        <form id="incidentForm" action="{{ route('incidents.store') }}" method="POST" class="space-y-8">
            @csrf
            ...
            <div class="bg-white p-6 rounded-lg shadow-md border">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <div>
                        <label for="incident_latitude" class="block text-sm font-medium text-gray-700">Latitude (Optional)</label>
                        <input type="text" id="incident_latitude" name="incident_latitude" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 p-2 border" placeholder="e.g., 14.5995">
                    </div>

                    <div>
                        <label for="incident_longitude" class="block text-sm font-medium text-gray-700">Longitude (Optional)</label>
                        <input type="text" id="incident_longitude" name="incident_longitude" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 p-2 border" placeholder="e.g., 120.9842">
                    </div>

                    <div class="col-span-1 md:col-span-2 lg:col-span-4">
                        <button type="button" onclick="getGeolocation()" class="w-full bg-green-500 text-white py-2 px-4 rounded-lg hover:bg-green-600 transition duration-150">
    ...
    <div id="message-modal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-75 overflow-y-auto h-full w-full z-50">
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div class="mt-3 text-center">
                <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">Form Submission</h3>
                <div class="mt-2 px-7 py-3">
                    <p class="text-sm text-gray-500" id="modal-content"></p>
                </div>
                <div class="items-center px-4 py-3">
                    <button id="modal-close-btn" class="px-4 py-2 bg-indigo-500 text-white text-base font-medium rounded-md w-full shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition">
                        OK
                    </button>
                </div>
            </div>
    ...
    <script>
        ...
        // Form submission handler (preventing actual submission and displaying data)
        document.getElementById('incidentForm').addEventListener('submit', function(event) {
            ...
            // In a real application, this is where you would make an AJAX/fetch request to your Laravel route.
            // Example success message:
            showMessage('Report Submitted', 'Thank you! The incident data has been collected and logged to the console.');
        ...
        function getGeolocation() {
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(
                    (position) => {
                        document.getElementById('incident_latitude').value = position.coords.latitude;
                        document.getElementById('incident_longitude').value = position.coords.longitude;
                        showMessage('Location Acquired', 'Latitude and Longitude have been automatically filled.');
                    },
                    (error) => {
                ...
                showMessage('Location Error', 'Geolocation is not supported by this browser.');
            }
        }
    </script>
    <x-partials.stack-js />
